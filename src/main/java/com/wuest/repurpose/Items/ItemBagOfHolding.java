...
public class ItemBagOfHolding extends Item {
                ...
                if (handler != null) {
                    ...
                    if (world.isAirBlock(newBlockPos)) {
                        ...
                        if (stackInSlot.getItem() instanceof BlockItem) {
                            ...
                            // world.
                            ActionResultType placementResult = this.PlaceBlockFromPouch(player, stackInSlot, result,
                                    hand);
    ...
    public ActionResultType PlaceBlockFromPouch(PlayerEntity player, ItemStack itemStack,
                                                BlockRayTraceResult rayTraceResult, Hand hand) {
        QuickItemUseContext itemUseContext = new QuickItemUseContext(player.getEntityWorld(), player, hand, itemStack, rayTraceResult);

        BlockItem itemBlock = (BlockItem) itemStack.getItem();
        return itemBlock.tryPlace(new BlockItemUseContext(itemUseContext));
    }
    ...
    /**
     * This is just a quick class to be able to use the constructor necessary.
     */
    public class QuickItemUseContext extends ItemUseContext {
        public QuickItemUseContext(World worldIn, @Nullable PlayerEntity player, Hand handIn, ItemStack heldItem, BlockRayTraceResult rayTraceResultIn) {
