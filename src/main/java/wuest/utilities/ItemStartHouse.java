...
public class ItemStartHouse extends Item {
	...
	private void BuildStructure(World world, BlockPos startingPosition) {
		...
		this.SetFloor(world, startingPosition, Blocks.dirt, 4,
		...
		// Create the floor.
		this.SetFloor(world, startingPosition, floor, 4,
				new ArrayList<ItemStack>());
	...
	private void BuildExterior(World world, BlockPos startingPosition,
			EntityPlayer player) {
		// Keep the corner positions since they are important.
	...
	private ArrayList<ItemStack> SetFloor(World world, BlockPos pos,
		...
		while (wallLength > 0) {
			for (int j = 0; j < 4; j++) {
				...
				for (int k = 0; k <= wallLength; k++) {
					// j is the north/south counter.
					this.ReplaceBlock(world, pos, stateWithoutFacing
	...
	private void PlaceAndFillChest(World world, BlockPos cornerPosition) {
		...
		if (this.currentConfiguration.addChestContents) {
			...
			if (tileEntity instanceof TileEntityChest) {
				...
				// Add the weapons.
				chestTile.setInventorySlotContents(0, new ItemStack(
						Items.stone_axe));
				chestTile.setInventorySlotContents(1, new ItemStack(
						Items.stone_hoe));
				chestTile.setInventorySlotContents(2, new ItemStack(
						Items.stone_pickaxe));
				chestTile.setInventorySlotContents(3, new ItemStack(
						Items.stone_shovel));
				chestTile.setInventorySlotContents(4, new ItemStack(
						Items.stone_sword));

				// Add the armor.
				chestTile.setInventorySlotContents(5, new ItemStack(
						Items.leather_boots));
				chestTile.setInventorySlotContents(6, new ItemStack(
						Items.leather_chestplate));
				chestTile.setInventorySlotContents(7, new ItemStack(
						Items.leather_helmet));
				chestTile.setInventorySlotContents(8, new ItemStack(
						Items.leather_leggings));

				// Add some bread.
				chestTile.setInventorySlotContents(9, new ItemStack(
						Items.bread, 20));

				// Add potatoes.
				chestTile.setInventorySlotContents(10, new ItemStack(
						Items.potato, 3));

				// Add carrots.
				chestTile.setInventorySlotContents(11, new ItemStack(
						Items.carrot, 3));
	...
	private void PlaceAndFillCraftingMachines(World world,
		...
		this.ReplaceBlock(world, itemPosition, Blocks.furnace.getDefaultState()
	...
	private void PlaceMineShaft(World world, BlockPos pos) {
		...
		// South wall.
		tempStacks.addAll(this.CreateWall(world, 3, 3, EnumFacing.EAST, pos
				.south(2).west(), Blocks.air));
		tempStacks.addAll(this.CreateWall(world, 3, 3, EnumFacing.EAST, pos
				.south(3).west(), Blocks.stone));

		// East wall.
		tempStacks.addAll(this.CreateWall(world, 3, 4, EnumFacing.NORTH, pos
				.south(2).east(), Blocks.air));
		tempStacks.addAll(this.CreateWall(world, 3, 4, EnumFacing.NORTH, pos
				.south(2).east(2), Blocks.stone));

		// North wall.
		tempStacks.addAll(this.CreateWall(world, 3, 3, EnumFacing.WEST, pos
				.north().east(), Blocks.air));
		tempStacks.addAll(this.CreateWall(world, 3, 3, EnumFacing.WEST, pos
				.north(2).east(), Blocks.stone));

		// West wall.
		tempStacks.addAll(this.CreateWall(world, 3, 4, EnumFacing.SOUTH, pos
	...
	private ArrayList<ItemStack> CreateLadderShaft(World world, BlockPos pos,
			...
			for (int i = 0; i < 4; i++) {
				...
				if (facing == EnumFacing.WEST && torchCounter == 6
					...
					if (!surroundingBlock.isBlockNormalCube()) {
						...
						// it with stone.
						this.ConsolidateDrops(surroundingBlock, world, tempPos,
	...
	private ArrayList<ItemStack> ConsolidateDrops(Block block, World world,
	...
	private ArrayList<ItemStack> CreateWall(World world, int height,
