...
public class BuildingMethods 
{
	 ...
	 * @param startingPosition - The starting position, note the width, depth will be used to determine the starting corner(s).
	...
	public static void SetCeiling(World world, BlockPos pos, Block block, int width, int depth, Block stairs, HouseConfiguration configuration) 
	{
		...
		while (wallLength > 0) {
			for (int j = 0; j < 4; j++) {
				...
				for (int k = 0; k <= wallLength; k++) {
					// j is the north/south counter.
					BuildingMethods.ReplaceBlock(world, pos, stateWithoutFacing
